...
| **I-12** | Three plain platform agents wired to a live web-search tool in one call each, confirmed on `office_roster`; a shared 2,602-char source set for every role in office 1. `assisted` vs `proxy` is the load-bearing choice. | live calls 2026-08-27 |
